...
@Component({
  selector: 'app-product-page-wep',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './product-page-wep.component.html',
  styleUrls: ['./product-page-wep.component.css']
})
export class ProductPageWEPComponent implements OnInit {
  products: Product[] = [];
  ...
  ngOnInit(): void {
    this.productService.getProducts().subscribe(data => {
      ...
      this.filteredProducts = this.products.filter(product => product.name.includes('WEP'));
    });
  }
  ...
  filterPhuHop(): void {
    this.filteredProducts = this.shuffleArray(this.products.filter(product => product.name.includes('WEP')));
    this.displayedCount = 8;
  }
  ...
  filterBanChay(): void {
    this.filteredProducts = this.products
      .filter(product => product.name.includes('WEP'))
