...
@Component({
  selector: 'app-product-page-wlhp',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './product-page-wlhp.component.html',
  styleUrls: ['./product-page-wlhp.component.css']
})
export class ProductPageWLHPComponent implements OnInit {
  products: Product[] = [];
  ...
  ngOnInit(): void {
    this.productService.getProducts().subscribe(data => {
      ...
      this.filteredProducts = this.products.filter(product => product.name.includes('WLHP'));
    });
  }
  ...
  filterPhuHop(): void {
    this.filteredProducts = this.shuffleArray(this.products.filter(product => product.name.includes('WLHP')));
    this.displayedCount = 8;
  }
  ...
  filterBanChay(): void {
    this.filteredProducts = this.products
      .filter(product => product.name.includes('WLHP'))
