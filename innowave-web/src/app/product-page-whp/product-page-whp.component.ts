...
import { ProductService, Product } from '../product.service';  // Đảm bảo bạn đã import đúng service
import { RouterModule } from '@angular/router'; 

@Component({
  selector: 'app-product-page-whp',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './product-page-whp.component.html',
  styleUrls: ['./product-page-whp.component.css']
})
export class ProductPageWHPComponent implements OnInit {
  products: Product[] = [];
  ...
  ngOnInit(): void {
    this.productService.getProducts().subscribe(data => {
      ...
      this.filteredProducts = this.products.filter(product => product.name.includes('WHP'));
    });
  }
  ...
  filterPhuHop(): void {
    this.filteredProducts = this.shuffleArray(this.products.filter(product => product.name.includes('WHP')));
    this.displayedCount = 8;
  }
  ...
  filterBanChay(): void {
    this.filteredProducts = this.products
      .filter(product => product.name.includes('WHP'))
